...
public class VagrantCliProvider implements Function<File, VagrantApiFacade<Box>> {
   ...
   static class VagrantCliFacade implements VagrantApiFacade<Box> {
      ...
      public VagrantCliFacade(File path, CommandIOListener wireLogger) {
   ...
   @Inject
   VagrantCliProvider(VagrantWireLogger wireLogger) {
      this.wireLogger = checkNotNull(wireLogger, "wireLogger");
   }

   @Override
   public VagrantApiFacade<Box> apply(File path) {
      return new VagrantCliFacade(path, wireLogger);
   }
